...
def scrape_cainz_product(jan_code):
    ...
    with sync_playwright() as p:
        ...
        if any(keyword in visible_text for keyword in PROHIBITED_KEYWORDS):
            browser.close()
            raise ValueError(
                "⚠️ TOS VIOLATION: Item contains prohibited keywords. Skipped automatically."
            )
        ...
        try:
            storage_match = re.search(
                r"収納.*?(\d+(?:\.\d+)?)\s*[×x*]\s*.*?(\d+(?:\.\d+)?)\s*[×x*]\s*.*?(\d+(?:\.\d+)?)\s*(cm|mm)?",
                content,
            )
            # Catch raw dimensions without labels: "63.5×35.9×38.3" or "直径17×深さ9"
            general_match = re.search(
                r"(\d+(?:\.\d+)?)\s*[×x*]\s*(?:奥行|D|深さ|縦)?\s*(\d+(?:\.\d+)?)\s*[×x*]\s*(?:高さ|H|厚さ)?\s*(\d+(?:\.\d+)?)\s*(cm|mm)?",
                content,
                re.IGNORECASE,
            )
        ...
        if clicked:
            try:
                ...
                sano_blocks = modal_soup.find_all(
                    lambda t: t.name in ["label", "li", "div"]
                    and t.get_text()
                    and re.search(r"カインズ佐野|佐野店資材館", t.get_text())
                )
